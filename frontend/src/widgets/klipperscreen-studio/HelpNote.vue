...
/** A collapsible "ℹ what's this?" note. Collapsed by default so it adds zero clutter to the
 *  default view; expands a dashed teaching block (copy from the i18n catalog + an optional
 *  illustration). The `glossary` topic renders the term list (after its body).
 */
...
const title = computed(() => tt(`klipperscreenStudio.help.topics.${props.topic}.title`))
const bodyKey = computed(() => `klipperscreenStudio.help.topics.${props.topic}.body`)
      ...
      <div class="min-w-0 flex-1 space-y-1 leading-snug opacity-80">
        ...
        <dl v-if="topic === 'glossary'" class="space-y-1">
          <div v-for="k in GLOSSARY_KEYS" :key="k">
            <dt class="font-bold">{{ tt(`klipperscreenStudio.help.glossary.${k}.term`) }}</dt>
            <dd>{{ tt(`klipperscreenStudio.help.glossary.${k}.def`) }}</dd>
          </div>
        </dl>
      </div>
    </div>
