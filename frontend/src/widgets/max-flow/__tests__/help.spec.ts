...
const ILLO_KEYS: HelpIlloKey[] = ['flow', 'stallguard', 'slip', 'safety']
const t = i18n.global.t as unknown as (key: string) => string
const te = i18n.global.te as unknown as (key: string) => boolean

describe('max-flow help structure + catalog', () => {
  it('has a title for every topic and a body for non-glossary topics', () => {
    for (const topic of HELP_TOPICS) {
      expect(te(`maxFlow.help.topics.${topic}.title`), topic).toBe(true)
      expect(t(`maxFlow.help.topics.${topic}.title`).length, topic).toBeGreaterThan(0)
      if (topic !== 'glossary') {
        expect(t(`maxFlow.help.topics.${topic}.body`).length, topic).toBeGreaterThan(10)
      }
    }
  })
  ...
  it('the glossary defines each term with a term + def', () => {
    for (const k of GLOSSARY_KEYS) {
      expect(t(`maxFlow.help.glossary.${k}.term`).length, k).toBeGreaterThan(0)
      expect(t(`maxFlow.help.glossary.${k}.def`).length, k).toBeGreaterThan(10)
    }
  })
})
