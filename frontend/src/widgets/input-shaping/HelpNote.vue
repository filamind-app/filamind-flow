...
const title = computed(() => tt(`inputShaping.help.topics.${props.topic}.title`))
const bodyKey = computed(() => `inputShaping.help.topics.${props.topic}.body`)
...
<template>
  <div class="font-mono text-[10px]">
    <button
      ...
      @click="open = !open"
    >
      {{ open ? '▾' : 'ℹ' }} {{ open ? title : t('inputShaping.help.whatsThis') }}
    </button>
    <div
      v-if="open"
      ...
      <div class="min-w-0 flex-1 space-y-1 leading-snug opacity-80">
        ...
        <dl v-if="topic === 'glossary'" class="space-y-1">
          <div v-for="k in GLOSSARY_KEYS" :key="k">
            <dt class="font-bold">{{ tt(`inputShaping.help.glossary.${k}.term`) }}</dt>
            <dd>{{ tt(`inputShaping.help.glossary.${k}.def`) }}</dd>
          </div>
        </dl>
      </div>
    </div>
