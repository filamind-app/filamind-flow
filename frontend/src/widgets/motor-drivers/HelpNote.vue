...
const title = computed(() => tt(`motorDrivers.help.topics.${props.topic}.title`))
const bodyKey = computed(() => `motorDrivers.help.topics.${props.topic}.body`)
      ...
      <div class="min-w-0 flex-1 space-y-1 leading-snug opacity-80">
        ...
        <dl v-if="topic === 'glossary'" class="space-y-1">
          <div v-for="k in GLOSSARY_KEYS" :key="k">
            <dt class="font-bold">{{ tt(`motorDrivers.help.glossary.${k}.term`) }}</dt>
            <dd>{{ tt(`motorDrivers.help.glossary.${k}.def`) }}</dd>
          </div>
        </dl>
      </div>
    </div>
