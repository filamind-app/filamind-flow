...
| POST   | `/api/maxflow/run`         | Run the live max-flow test (home + center → heat → StallGuard sanity pre-check → ramp + sample); preflight (chopper-mode/StallGuard) + SG4 bias-floor + ≥180 °C; aborts before the ramp if the live SG signal is unusable; refused while printing; heater always cut. |
