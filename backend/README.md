...
| GET    | `/api/topology`            | Host → MCU topology from the live config: the host SBC (identified via `/machine/system_info`, linked to a catalog host) + each MCU's connection (CAN/USB/UART) + chip (linked to a catalog MCU) + suggested board + the components (steppers/drivers/heaters/fans/sensors) on it, attached by each pin's chip prefix (read-only). |
