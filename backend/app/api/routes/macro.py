...
class SimulateRequest(BaseModel):
    """Body for ``POST /macro/simulate`` — the G-code to simulate."""

    gcode: str = Field(..., description="Literal G-code program to simulate")


@router.post("/simulate")
async def macro_simulate(body: SimulateRequest) -> dict[str, Any]:
    """Simulate a G-code program → path / bounds / totals / timeline (pure)."""
    return gcode_sim.simulate(body.gcode)
