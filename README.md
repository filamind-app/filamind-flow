...
| **Motor Drivers** | A live inventory of every TMC stepper driver, read straight from the Klipper config — run/hold current, chopper mode, microsteps, StallGuard, temperature, and health, each annotated with authoritative per-model facts from a built-in capability map. Assign each axis its motor from a 200+ motor catalog, get recommended run current + driver registers from the motor's datasheet (a faithful `motor_constants` port), and copy-to-config or apply them live behind a confirm (reversible; refused while printing). Glossary + illustrated help. Generic across all printers and TMC models. | 🚧 P1–P4 (dashboard · capability map · picker · recommender · apply) |
