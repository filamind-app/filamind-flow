...
| **Board Topology** | A live map of how your control boards connect — the host (SBC) and every MCU it talks to, each with its connection type (USB / CAN bus / UART), chip, and a best-effort board guess, read live from the config. Each MCU lists the **components on it** (steppers / drivers / heaters / fans / sensors) and links to its **board record + manufacturer / MCU / drivers in the hardware database** (click through to the Hardware Browser). You can **confirm or override the detected board** per MCU — your choice is saved on the host and reused on every read. Illustrated help. Generic across all Klipper printers. | ✅ Shipped |
