...
| **KlipperScreen Studio** | Take charge of the printer's physical touchscreen without the command line. Edit `KlipperScreen.conf` through the same gated save (backup first, busy-refusal, stale-write guard) and one-click restart — raw, or via a **Settings form** for the common `[main]` options (theme, language, screen-sleep, font size, 24-hour clock, DPMS, cursor, e-stop confirm…) that writes only what you change. Or design the screen visually: a **theme builder** that picks colors per palette token + corner radius with a live preview and writes a real theme to the screen, and a **visual menu editor** that lays out each screen's button grids (`__main` / `__print` / `__splashscreen`) as an add / rename / reorder / nest tree where every button **opens a sub-menu**, **opens a screen** (built-in panels), or **runs G-code**. For total control there's also a **Kiosk** mode that turns the touchscreen into FilaMind Flow itself (a fullscreen browser) — a reversible swap with KlipperScreen (temporary by default, optional make-default, auto-recovers a dark screen) installed once with `scripts/install.sh kiosk`. A glossary + per-tab illustrated help; runs on the printer host. | ✅ Shipped (conf + settings + theme builder + menu editor + kiosk) |
