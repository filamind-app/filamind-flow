...
| 🚧 **A3 — Board Topology** | Auto board/MCU detection + a visual SBC↔mainboard↔toolhead topology graph. **Detection backend ✅ v0.99.0** (`GET /api/topology` — each MCU's connection CAN/USB/UART + chip/board guess from the live config). 📋 Next: the topology-graph UI + a pin-conflict validator (best paired with the Config Editor). | A | low (read-only) |
