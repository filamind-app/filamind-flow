...
| ✅ **A4 — Hardware Browser + Templates** | Searchable unified hardware DB (~3,047 components after the audit data-clean) + an insertable config/macro template library. **Data + search backend ✅ v0.103.0** · **Browser UI ✅ v0.104.0** · **Config Templates library ✅ v0.105.0** · **DB data-quality clean ✅ v0.108.0** (0 unnamed items, manufacturer-directory rows removed, deduped, CI gates). 📋 Follow-up: reconcile item manufacturer strings against the 274-row directory + cross-ref keys to motor/driver catalogs. | A | low (read-only) |
