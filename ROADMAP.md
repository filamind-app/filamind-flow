...
| **3 — RTL + Arabic** | Arabic catalog shipped (all 845 keys) + 6-form plural rule + `latn` digits; `<html dir/lang>` flips to RTL on switch. **Remaining (RTL polish):** the logical-property sweep, `[dir=rtl]` brutalist tweaks, bidi-isolated measurements, self-hosted Arabic webfonts. | 🚧 v0.84.0 (content) |
