...
| 🚧 **A1 — Config Editor** | Full schema-driven `printer.cfg` editor: 92 sections / 847 params, live validation, byte-faithful save, auto-backup + diff. **Read path ✅ v0.94.0** (`/api/config/files` + `/api/config/file` — list + parse the live config into sections → params + issues). 📋 Next: the viewer UI, then the gated save path. | A | medium (writes cfg) |
