...
| 🚧 **B1 — Max-Flow** | Measure the real max volumetric flow (mm³/s) via TMC StallGuard slip detection, + 80/90 % slicer values. **Analysis core ✅ v0.93.0** (`max_flow` slip detectors). **Planner ✅ v0.97.0** (`/api/maxflow/plan` — pure flow→feedrate ramp preview + slicer recommendations + safety bounds). 📋 Next: the gated measurement loop (heat → extrude → sample SG) + UI + live run. | B | high (heat + motion) |
