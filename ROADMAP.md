...
| ✅ **A4 — Hardware Browser + Templates** | Searchable unified hardware DB (~3,643 components) + an insertable config/macro template library. **Data + search backend ✅ v0.103.0** · **Browser UI ✅ v0.104.0** · **Config Templates library ✅ v0.105.0** · **DB restore + clean ✅ v0.109.0** (restored 577 stepper products + 20 wiring rows the v0.108.0 over-clean had dropped; manufacturer directory kept in `manufacturers[]`; scrubbed all external-project refs incl. klippain/GPL-3; CI per-category floors so a regen can't gut a category). **DB overhaul in progress** (phased): board `ports[]` aggregation ✅ → **web enrichment ✅ v0.126.0 (all 300 standalone control boards enriched with specs, media links, config-affecting electronics & Klipper config notes)** → media (images/schematics/pinouts) → Board-Topology/Max-Flow links → sectioned catalog. 📋 Remaining: printer-model preset enrichment (108 entries), section catalog, topology/max-flow cross-links. | A | medium |
