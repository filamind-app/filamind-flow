...
| 🚧 **A2 — Macro Designer** | Offline G-code/macro simulator. **Simulator core ✅ v0.101.0** (`POST /api/macro/simulate`). **Editor UI ✅ v0.102.0** (G-code editor → 2D path canvas + bounds/totals/time + timeline + built-in macro library, i18n ×7). 📋 Next: Jinja / macro expansion (`{...}` vars + `{% %}`). | A | low (no motion) |
...
| 🚧 **A4 — Hardware Browser + Templates** | Searchable unified hardware DB (3,671 components) + an insertable config/macro template library. **Data + search backend ✅ v0.103.0** (`/api/hardware` — 3,671 items / 274 manufacturers / 13 categories, free-text + category + manufacturer filters, paginated). 📋 Next: the browser UI, then the template library. | A | low (read-only) |
