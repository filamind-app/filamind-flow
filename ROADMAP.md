...
| 🚧 **A1 — Config Editor** | Full schema-driven `printer.cfg` editor: 92 sections / 847 params, live validation, byte-faithful save, auto-backup + diff. **Read path ✅ v0.94.0** (`/api/config/files` + `/api/config/file`). **Viewer UI ✅ v0.95.0** (file picker → collapsible sections → params, validation banner, structured/raw views, illustrated help, i18n ×7). 📋 Next: the gated save path (edit → backup → write → restart). | A | medium (writes cfg) |
