...
| 📋 **10 — Advanced flash** | 1200bps magic-baud reboot **+ "Test DFU Cycle" validation** (gates auto-DFU / auto-exit), DFU retry + `:leave` + auto-exit, USB-to-CAN bridge handling, AVR auto-detect, **post-flash re-enumeration tracking** (board id changes after flash → auto-update the registry), **Katapult native wire-protocol** return-to-firmware + CAN interface bring-up | reliability on more boards |
