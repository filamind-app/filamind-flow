...
| 🚧 **A2 — Macro Designer** | Offline G-code/macro simulator. **Simulator core ✅ v0.101.0** (`POST /api/macro/simulate` — parse G0/G1/G90/G91/M82/M83/G92/G28 → 2D path + bounds + travel/extrusion + time estimate + timeline). 📋 Next: the editor UI (2D path view + built-in macro library), then Jinja/macro expansion. | A | low (no motion) |
