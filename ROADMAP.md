...
| 🚧 **A4 — Hardware Browser + Templates** | Searchable unified hardware DB (3,671 components) + an insertable config/macro template library. **Data + search backend ✅ v0.103.0** · **Browser UI ✅ v0.104.0** (search + category/manufacturer filters + paginated spec-sheet results, i18n ×7). 📋 Next: the insertable config/macro template library. | A | low (read-only) |
