...
## Hardware DB — cross-widget data backbone (planned)
...
purely additive** (existing endpoints/fields untouched). _Analysis only — not yet built._

| Phase | Scope | Risk |
| ----- | ----- | ---- |
| 📋 **DB-1 — Performance foundation** (invisible) | Build `id→entity` index dicts + precompute each entity's lowercased search haystack at load (turns the flat search ~16× faster and every `*_by_id` O(1)); minify the dataset in the build (−28% parse); `ETag` / `Cache-Control: immutable` on the read-only responses (kills card-expand re-fetches); **UI quick-win: catalog tile counts → canonical** (fixes the raw-1357 vs canonical-380 mismatch). | low (read-only, strictly non-regressing) |
| 📋 **DB-2 — Linking backbone** | A `manufacturer_id` + alias table (brand join ~51%→~100%); **MCU promoted to a first-class entity** (the board↔mcu↔firmware hub); a **precomputed `links[]` edge table** (composite keys `<type>:<id>`, loaded into in-memory adjacency) with typed relations (control_board, mcu, onboard/compatible_driver, host_carrier, recommended_driver, accelerometer, probe, nozzle_std, flow_table, stallguard_profile, manufacturer); a new `GET /api/hardware/{type}/{id}/related` (+ `?expand=`) at O(1); a **CI edge-validator** so links are derived, never rot. | medium (additive) |
| 📋 **DB-3 — UI reuse + cross-links** | Extract one shared `EntityCatalog` component (collapse the 5 near-clone panels); surface the **faceted filters** the backend already accepts (`board_class` / `nema` / `kind` / `manufacturer`); a reusable **`HardwarePicker`** (generalize `MotorPicker`/`ComboSelect`) so any widget can embed "pick a part" + auto-fill (e.g. Motor Drivers auto-fills `run_current`); **cross-link chips** ("used by / compatible with", click → jump to the related entity); deep-link/hash routing to an entity. | medium |
| 📋 **DB-4 — Polish + convergence** | Images/thumbnails from `media.imageUrl` + an optional detail drawer; converge the Motor-Drivers `motor_catalog`/`driver_catalog` silos onto the hardware DB. | low |
