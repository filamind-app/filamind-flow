...
| ✅ **13 - Health & install integrity** | `/firmware/health` checks (sudoers, **udev DFU rule**, sudo-ready, dfu-util) + a UI **setup** badge with fix-it hints; `scripts/install.sh sudoers` now ships **`99-stm32-dfu.rules`** so DFU flashes without `sudo`, plus **`99-filamind-canbus.rules`** + a oneshot service that **auto-recover the CAN bus** when the USB-CAN adapter is (re)plugged (bring the interface up + reconnect Klipper only if it is stuck) - so the one physical replug a U2C flash needs (its USB hub can't cycle port power in software) is the *only* step; both the sudoers grant and this recovery self-heal on update. (Startup self-heal needs root the backend doesn't have - the badge tells the user the one command to run instead.) | DFU-without-sudo + auto CAN recovery + a clear setup light |
...
| 🟡 **C1 - KlipperScreen Studio** | Take charge of the printer's physical touchscreen. **Conf editor ✅** (read/edit `KlipperScreen.conf` through the gated save - backup + busy-refusal + stale-write guard - and a one-click restart). **Theme builder ✅** (per-token colors + corner radius with a live preview → writes a real theme to the screen's `styles/`). **Graphical options (backend) ✅** + **menu-tree (backend) ✅**. **Visual menu editor ✅** (add / rename / reorder / nest the `__main` / `__print` / `__splashscreen` button grids; each button opens a sub-menu / opens a screen / runs G-code). **FilaMind Kiosk ✅** - total screen control: `scripts/install.sh kiosk` installs a Wayland kiosk (cage + Chromium) + a `filamind-kiosk` unit that conflicts with KlipperScreen; `/api/screen/kiosk*` reversibly swaps the touchscreen between the two (temporary by default, optional persist; auto-recovers a dark screen); and the widget's **Kiosk tab** drives it (mode display + setup hint + Switch/Restore behind a confirm). **Settings form ✅** - the common `[main]` options as friendly controls (theme / language / screen-sleep / font-size + toggles), pre-filled, writing only what changed. **Widget complete: config · settings · menus · themes · kiosk.** | C | medium (writes host files + restarts the screen) |
| 🚧 **A5 - Board Topology v2** | Deeper, live machine modeling. **T1 - Live MCU identity ✅** - query the running `mcu` objects for each MCU's authoritative chip (`mcu_constants.MCU`), firmware (`mcu_version`) and CAN stats, so a CAN toolhead (whose config signature is only a uuid) gets a real chip + the board fingerprint is narrowed to boards that carry that chip; firmware shown per MCU. **T2 - CAN-adapter (U2C) node ✅** - from `machine_system_info.canbus` (gs_usb + bitrate), surfaced as a clickable **node on the CAN backbone** (and a header row) with a best-effort, unconfirmed catalog U2C link (a gs_usb dongle is not a Klipper MCU, so it had no node otherwise). Confirm/change its exact type (U2C variant), and a **host-side flash that enters DFU in software** (a `dfu-util -e` detach - candleLight/budgetcan expose a DFU runtime interface - with the manual BOOT-button entry as the fallback) then dfu-util of the official BTT firmware, gated/print-locked. **T3 - Display detection (3 layers) ✅** - the EXP-header LCD (a Mini12864) attaches to its MCU as a `display` component; KlipperScreen (a host service) surfaces as a host touch display; a KNOMI is detected from its companion macros as a host WiFi display. 📋 **T4 - Manual part add/edit** - a `topology_additions` store + NodeInspector UI to place catalog parts the config can't expose (U2C / display / KNOMI / accessory), folded into the snapshot/diff. | A | low-med (one new write path) |
| 📋 **Host Control - CAN bus control** | A CAN panel: `can0` state / bitrate / error counters + node listing (reusing the firmware-flash CAN enumeration), and **full control** - bring the interface up/down, set bitrate, restart - gated by a confirm and refused while printing. Adds a narrow `ip link` rule to the passwordless-sudo grant. | C | high (root + interface) |
| 📋 **Host Control - Health advisor** | Read-only advisory cards from the topology/system scan: redundant remote-access services, dev-channel Klipper well ahead of stable, an open `trusted_clients`, resource pressure, failed units. Pairs with Config-Editor lint for duplicate keys + conflicting adaptive-mesh includes. | C | low |
| ✅ **Host Control - Boot parameters ✅ v1.20.0** | A **Boot params** tab to edit the host's boot config (device-tree overlays + kernel args) - platform-aware over `/boot/armbianEnv.txt` (Armbian: BTT CB1/CB2 + RK/Allwinner) and `config.txt` (Raspberry Pi). **Curated capability cards** (SPI accelerometer / I2C / extra UART / CAN MCP2515 / WS2812 / RTC + console / CMA / GPU-overclock / USB-current) with plain-language help + caveats, plus a raw **Advanced** overlay/param editor. Every edit is **staged → unified-diff preview → gated apply**, **timestamped-backed-up** before any write, **revertable** to the last backup, and needs a **reboot** (typed-confirm, print-locked, never automatic). Minimal-diff parsers preserve every untouched line; writes reuse the granted `sudo cp` (path-guarded) - **no new sudoers grant, no manual commands**. Localised ×19. | C | medium (writes /boot) |
