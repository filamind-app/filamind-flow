...
purely additive** (existing endpoints/fields untouched). _DB-1 + DB-2 shipped; DB-3/DB-4 planned._
...
| 🟡 **DB-3 — UI reuse + cross-links** | **DB-3a ✅ v0.140.0** — **cross-link chips** on every entity (manufacturer / MCU / on-board+supported drivers, click → jump to the related entity, in-widget deep-linking) + two new browse tabs (**Brands**, **MCUs**) + illustrated help, localised ×7. **DB-3b 📋** — extract one shared `EntityCatalog` component (collapse the now-7 near-clone panels); surface the **faceted filters** the backend already accepts (`board_class` / `nema` / `kind` / `manufacturer`); a reusable **`HardwarePicker`** (generalize `MotorPicker`/`ComboSelect`) so any widget can embed "pick a part" + auto-fill (e.g. Motor Drivers auto-fills `run_current`). | medium |
